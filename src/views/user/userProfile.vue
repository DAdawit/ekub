        ...
        <v-row class="mx-3">
            <v-col cols="12" sm="12" md="12" lg="4">
                <v-card>
                    ...
                    <h3 class="d-flex justify-center">{{user.first_name}} {{user.last_name}}</h3>
                ...
                <v-card>
                    ...
                    <v-alert icon="mdi-shield-lock-outline" prominent text type="info">
                        ...
                        Please fill this form if you are the guarantor standing on behalf of : Helen Hailu.
                    </v-alert>
                    <v-form class="pb-3 ">
                    ...
                    <ValidationObserver v-slot="{ handleSubmit }">
                        <form @submit.prevent="handleSubmit(ResetPassword)">
                            <ValidationProvider rules="required|email" name="E-mail" v-slot="{ errors }">
                                <v-text-field dense label="E-mail" placeholder="Current Password" outlined
                                    v-model="user.email" class="d-flex justify-center mx-2" :error="errors.length > 0"
                        ...
                        <form @submit.prevent="handleSubmit(updatePro)">
                            <v-row class="pb-3">
                                <v-col cols="12" sm="12" md="12" lg="6">
                                    <ValidationProvider rules="required" name="account name" v-slot="{ errors }">
                                        <v-text-field dense label="account_name" outlined class="mx-2"
                                            v-model="userProfile.account_name" :error="errors.length > 0"
                                ...
                                <v-col cols="12" sm="12" md="12" lg="6">
                                    <ValidationProvider rules="required|integer" name="account number"
                                        ...
                                        <v-text-field dense label="account_number" outlined class="mx-2"
                                            v-model="userProfile.account_number" :error="errors.length > 0"
                                            :error-messages="errors[0]"></v-text-field>
                                    </ValidationProvider>
                                </v-col>
                                <v-col cols="12" sm="12" md="12" lg="6">
                                    <ValidationProvider rules="required" name="payment type" v-slot="{ errors }">
                                        <v-select v-model="userProfile.payment_type" :items="paymentType"
                                            item-text="name" item-value="id" label="payment type*" single-line outlined
                                            dense :error="errors.length > 0" :error-messages="errors[0]">
                                        </v-select>
                                    </ValidationProvider>
                                </v-col>
                                <v-col cols="12" sm="12" md="12" lg="6">
                                    <v-menu ref="menu" v-model="menu" :close-on-content-click="false"
                                        :return-value.sync="userProfile.birth_date" transition="scale-transition"
                                        offset-y min-width="290px">
                                        <template v-slot:activator="{ on, attrs }">
                                            <v-text-field v-model="userProfile.birth_date" label="birth date"
                                                prepend-inner-icon="event" outlined dense class="mx-2" v-bind="attrs"
                                        ...
                                        <v-date-picker v-model="userProfile.birth_date" no-title scrollable>
                                            ...
                                            <v-btn text color="primary"
                                                @click="$refs.menu.save(userProfile.birth_date)">OK</v-btn>
                                        </v-date-picker>
                                    </v-menu>
                                </v-col>
                                <v-col cols="12" sm="12" md="12" lg="6">
                                    <ValidationProvider rules="required" name="Country" v-slot="{ errors }">
                                        <v-select :items="['ethiopia','Eirtria','sudan']" label="Country" outlined dense
                                            v-model="userProfile.country" class="mx-2" :error="errors.length > 0"
                                            :error-messages="errors[0]">
                                        </v-select>
                                    </ValidationProvider>
                                </v-col>
                                <v-col cols="12" sm="12" md="12" lg="6">
                                    <ValidationProvider rules="required" name="city" v-slot="{ errors }">
                                        <v-text-field dense label="State" outlined class="mx-2"
                                            v-model="userProfile.city" :value="userProfile.city"
                                ...
                                <v-row class="d-flex justify-content-center">
                                    <v-col cols="4">
                                        <v-btn color="purple" class="white--text" :loading="loading" type="submit"
                                            block>save
                                        </v-btn>
                                    </v-col>
                                </v-row>
                                ...
                                <v-col cols="12" sm="12" md="12" lg="12">
                                    <v-alert text type="info">
                                        <h3>IMPORTANT NOTICE!</h3>
                                        <p>Please do ensure that you upload a valid and clear passport photograph of
                                            your face above.</p>
                                        <p class="overline">THIS IS VERY VITAL BECAUSE ANY OTHER PHOTOGRAPH COULD
                                            LEAD
